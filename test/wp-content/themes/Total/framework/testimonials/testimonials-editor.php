<?php
/**
 * Used for renaming the Testimonials post type
 *
 * @package		Total
...
// Start Class
if ( ! class_exists( 'WPEX_Testimonials_Editor' ) ) {
	class WPEX_Testimonials_Editor {
		...
		public function __construct() {
			...
			add_action( 'wpex_testimonials_args', array( $this,'posttype_args' ) );

			// Filter the post type category arguments with custom values
			add_action( 'wpex_taxonomy_testimonials_category_args', array( $this,'cat_args' ) );

			// Filter the post type tag arguments with custom values
			add_action( 'wpex_taxonomy_testimonials_tag_args', array( $this,'tag_args' ) );
		...
		function posttype_args( $args ) {
			...
			// Labels
			$option = get_theme_mod( 'testimonials_labels' );
			if ( $option && 'Testimonials' != $option ) {
			...
			// Singular name
			$option = get_theme_mod( 'testimonials_singular_name' );
			if ( $option && 'Testimonial' != $option ) {
				$args['labels']['singular_name'] = $option;
			}

			// Slug
			$option = get_theme_mod( 'testimonials_slug', 'testimonial' );
			if ( $option && 'testimonial' != $option ) {
				$args['rewrite'] = array( "slug" => $option );
			}

			// Admin Icon
			if ( ( $option = get_theme_mod( 'testimonials_admin_icon' ) ) && ! is_array( $option ) ) {
				$args['menu_icon'] = 'dashicons-'. $option;
			}

			// Search
			if ( ! get_theme_mod( 'testimonials_search', true ) ) {
		...
		function cat_args( $args ) {
			...
			// Labels
			$option = get_theme_mod( 'testimonials_cat_labels', 'Testimonials Categories' );
			if ( $option && 'Testimonials Categories' != $option ) {
				$args['labels']['name']							= $option;
				$args['labels']['singular_name']				= $option;
			...
			// Custom Slug
			$option = get_theme_mod( 'testimonials_cat_slug', 'testimonials-category' );
			if ( $option && 'testimonials-category' != $option ) {
				$args['rewrite'] = array( "slug" => $option );
			}
		...
		function tag_args( $args ) {
			...
			// Labels
			$option = get_theme_mod( 'testimonials_tag_labels', 'Testimonials Tags' );
			if ( ! empty( $option ) && 'Testimonials Tags' != $option ) {
				$args['labels']['name']							= $option;
				$args['labels']['singular_name']				= $option;
				$args['labels']['search_items']					= __( 'Search','wpex');
				$args['labels']['popular_items']				= __( 'Popular','wpex');
			...
			// Custom Slug
			$option = get_theme_mod( 'testimonials_tag_slug', 'testimonials-tags' );
			if ( ! empty( $option ) && 'testimonials-tags' != $option ) {
				$args['rewrite'] = array( "slug" => $option );
			}
