<?php
/**
 * Registers the woocommerce carousel shortcode and adds it to the Visual Composer
 *
 * @package		Total
...
if ( !function_exists( 'vcex_woocommerce_carousel_shortcode' ) ) {
	function vcex_woocommerce_carousel_shortcode( $atts ) {
		...
		if ( $include_categories ) {
			$include_categories		= str_ireplace( ' ', '', $include_categories ); // remove blank spaces
			$include_categories		= explode( ',', $include_categories ); // turn into array
		...
		if ( $exclude_categories ) {
			$exclude_categories	= str_ireplace( ' ', '', $exclude_categories ); // remove blank spaces
			$exclude_categories	= explode( ',', $exclude_categories ); // turn into array
		...
		if ( $featured_products_only ) {
			$query_args['meta_key']		=  '_featured';
			$query_args['meta_value']	= 'yes';
		}

		// Exclude products our of stock
		if ( $exclude_products_out_of_stock ) {
		...
		if ( $wpex_query->posts ) :
			...
			// Main Classes
			$main_classes = 'wpex-carousel wpex-carousel-woocommerce owl-carousel clr';
			if ( $style ) {
							...
							<div class="wpex-carousel-entry-details clr"<?php echo $content_style; ?>>
								...
								if ( 'true' == $price ) {
									...
									$product_price = $product->get_price_html(); ?>
										<div class="wpex-carousel-entry-price"><?php echo $product_price; ?></div>
								<?php } ?>
...
if ( ! function_exists( 'vcex_woocommerce_carousel_shortcode_vc_map' ) ) {
	function vcex_woocommerce_carousel_shortcode_vc_map() {
		...
		// Useful arrays
		$text_transform = wpex_text_transform_array();
		...
		vc_map( array(
			...
			'icon' 					=> 'vcex-woocommerce-carousel',
			'params'				=> array(
...
add_action( 'vc_before_init', 'vcex_woocommerce_carousel_shortcode_vc_map' );